...
class BalanceWithdrawRequestAdmin(admin.ModelAdmin):
    list_display = ('user', 'get_user_balance', 'amount', 'transfer_type', 'transfer_number','risk_score', 'transfer_status')
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj and obj.transfer_status not in editable_statuses:
            return [field.name for field in self.model._meta.fields]
        return self.readonly_fields

    @admin.action(description="Approve selected withdrawal requests")
    def approve_requests(self, request, queryset):
        pending_requests = queryset.filter(transfer_status=BalanceWithdrawRequest.TransferStatus.AWAITING_APPROVAL)
        
        if not pending_requests.exists():
            self.message_user(request, "No requests in 'AWAITING_APPROVAL' state were selected.", messages.WARNING)
            return

        for withdrawal_request in pending_requests:
            try:
                BalanceService.approve_withdrawal(withdrawal_request, request.user.id, "Auto-approved via Admin Action")
            except Exception as e:
                self.message_user(request, f"Error approving request {withdrawal_request.id}: {e}", messages.ERROR)
        
        self.message_user(request, "Selected requests have been approved.", messages.SUCCESS)
        ...
        if 'apply' in request.POST:
            ...
            if form.is_valid():
                ...
                for req in pending_requests:
                    try:
                        BalanceService.reject_withdrawal(req, request.user.id, admin_notes)
                    except Exception as e:
