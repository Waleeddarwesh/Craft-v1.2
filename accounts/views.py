...
class SuppliersList(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if getattr(self, 'swagger_fake_view', False) or not user.is_authenticated:
            return Supplier.objects.filter(user__is_verified=True, user__is_active=True).order_by('id')
...
class TrendingSuppliersAPIView(APIView):
    ...
    @method_decorator(cache_page(60 * 15))
    def get(self, request, format=None):
        trending_suppliers = Supplier.objects.filter(user__is_verified=True, user__is_active=True).order_by('-Rating', '-Orders')[:10]
    ...
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            if user.last_password_reset_request is not None and now() - user.last_password_reset_request < timedelta(minutes=1):
                return Response({"message": "Please wait 1 minute before attempting another password reset."},
